...
**Referência visual travada:** o gerador em produção só libera os materiais depois de o administrador instalar a imagem JPEG EXATA da Opção B aprovada em **Equipe → Colaboradores → Instalar matriz visual aprovada**. O sistema confere seu SHA-256; não aceita versão redesenhada, recomposta, comprimida nem uma imagem de outra opção. A matriz fica salva no PostgreSQL. A frente do cartão preserva literalmente os pixels do quadro de referência; assinatura e verso preservam a arte-base, alterando apenas nome, função e contatos e o QR Code funcional para o endereço cadastrado. Quando a referência não está instalada, o app mostra a pendência e não exporta a antiga ilustração aproximada.

**Limite material da referência:** a montagem aprovada contém cartões muito mais horizontais que o padrão gráfico 90 × 50 mm. O PDF preserva as proporções da imagem (sem esticar), centralizada na área de 96 × 56 mm; há preenchimento nas sobras verticais. A fonte de referência é uma imagem de tela, e não uma arte vetorial de 300 dpi. Antes de impressão comercial definitiva, verificar qualidade e aprovar prova da gráfica. A imagem da assinatura HTML é incorporada como data URI, bloqueada por alguns clientes de e-mail; nesses casos use PNG ou hospede a imagem em HTTPS.
